...
func Index(w http.ResponseWriter, r *http.Request) {
	...
	if path == "" {
		...
		if err := views.Tmpl.ExecuteTemplate(&response, "index.html", data); err != nil {
		...
		var response bytes.Buffer
		if err := views.Tmpl.ExecuteTemplate(&response, "single.html", data); err != nil {
			log.Fatal(err)
			http.Error(w, http.StatusText(http.StatusNotFound), http.StatusNotFound)
			return
		}
...
func Register(w http.ResponseWriter, r *http.Request) {
	...
	case http.MethodGet:
		var response bytes.Buffer
		if err := views.Tmpl.ExecuteTemplate(&response, "register.html", nil); err != nil {
			http.Error(w, http.StatusText(http.StatusNotFound), http.StatusNotFound)
			return
		}
...
func Login(w http.ResponseWriter, r *http.Request) {
	...
	case http.MethodGet:
		var response bytes.Buffer
		if err := views.Tmpl.ExecuteTemplate(&response, "login.html", nil); err != nil {
			http.Error(w, http.StatusText(http.StatusNotFound), http.StatusNotFound)
			return
		}
	...
	case http.MethodGet:
		...
		case "article":
			if delete != "" && id != 0 {
				...
				var response bytes.Buffer
				if err := views.Tmpl.ExecuteTemplate(&response, "article.html", data); err != nil {
					http.Error(w, http.StatusText(http.StatusNotFound), http.StatusNotFound)
					return
				}
		...
		default:
			...
			var response bytes.Buffer
			if err := views.Tmpl.ExecuteTemplate(&response, "dashboard.html", data); err != nil {
				http.Error(w, http.StatusText(http.StatusNotFound), http.StatusNotFound)
				return
			}
